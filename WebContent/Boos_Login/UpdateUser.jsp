<%@page import="com.etc.entity.Login"%>
...
$(function(){
	$(".confirmAlt").click(function(){
	 var name=$(this).parent().parent().find("input[name='LoginId']").val();
	 var post=$(this).parent().parent().find("input[name='LoginPwd']").val();
	 var userId=$(this).next("input[name='UserId']").val();
	 $.ajax({
			url:"/attendance/UpdateLoginServlet",
			type:"get",
			data:{"LoginId":name,
				  "LoginPwd":post,
				  "UserId":userId},
...
<%
	List<Login> listUser =(List<Login>)session.getAttribute("ListUser");
	int flag = 10;
...
if(flag==1){
	// flag=1进入全部查询servlet并返回全部信息
	if(listUser==null){
		response.sendRedirect("/attendance/QueryUserServlet?flag=2");
		return;
	}
}
//flag为其他值的时候进入根据ID查询界面
	if (listUser == null ) {
		response.sendRedirect("/attendance/ByIdQueryLoginServlet?flag=1");
		return;
	}
%>
			...
			<div class="col-md-12 span_3">
			  <div class="bs-example1" data-example-id="contextual-table">
			    <table class="table table-hover">
			      ...
			      %>
			        <tr>
			          <td><%=listUser.get(0).getLoginId() %><input type="text" class="inputabc" name="LoginId"/></td>
			          <td><%=listUser.get(0).getLoginPwd() %><input type="text" class="inputabc" name="LoginPwd"/></td>
			          <td><%=listUser.get(0).getUserId() %></td>
			          <td><a href="" class="confirmAlt">确认修改</a><input type="hidden" value="<%=listUser.get(0).getUserId() %>" name="UserId"/></td>
			        </tr>
			        <%} %>
			        ...
			        <%if(flag==9){
			        	//flag=9进入for循环 显示全部查询记录
			        	for(int i=0;i<listUser.size();i++){
			        %>
			         <tr>
			          <td><%=listUser.get(i).getLoginId() %><input type="text" class="inputabc" name="LoginId"/></td>
			          <td><%=listUser.get(i).getLoginPwd() %><input type="text" class="inputabc" name="LoginPwd"/></td>
			          <td><%=listUser.get(i).getUserId() %></td>
			          <td><a href="" class="confirmAlt">确认修改</a><input type="hidden" value="<%=listUser.get(i).getUserId() %>" name="UserId"/></td>
			        </tr>
			        <%} 
			        }
			        %>
