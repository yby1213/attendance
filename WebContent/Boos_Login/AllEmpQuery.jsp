...
<script type="text/javascript">
	function aa(){
		...
		$.ajax({
			url:"/attendance/ListUserServlet",
			type:"get",
			data:{
				"username":userName,
				"flag":1/*进入ListUserServlet，并将单条list取到*/
			},
     ...
     <!-- Navigation -->
        <nav class="top1 navbar navbar-default navbar-static-top" role="navigation" style="margin-bottom: 0">
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="index.jps">考勤打卡系统</a>
            </div>
            <!-- /.navbar-header -->
			...
			<div class="col-md-12 span_3">
			  <div class="bs-example1" data-example-id="contextual-table">
			    <table class="table table-hover">
			      ...
			      <tbody>
			      <%
						List<User> listUsers =(List<User>)session.getAttribute("ListAll");
						if(listUsers==null){
							response.sendRedirect("/attendance/ListAllServlet?flag=1");
							return;
						}
					%>
			      ...
			      %>
			        <tr>
			          <th><%=listUsers.get(i).getUserId() %></th>
			          <td><%=listUsers.get(i).getUserName() %></td>
			          <td><%=listUsers.get(i).getUserPost()%></td>
			          <td><%=listUsers.get(i).getUserSal() %></td>
			          <td><%=listUsers.get(i).getClockNum() %></td>
			          <td><%=listUsers.get(i).getEntry_time()%></td>
			          
			          <td><a href="Mes.jsp?flag=3&userId=<%=listUsers.get(i).getUserId() %>">点击跳转</a></td>
			          <td><a href="DelEmp.jsp?userId=<%=listUsers.get(i).getUserId() %>&flag=0">点击跳转</a></td>
			          <td><a href="UpateEmp.jsp?userId=<%=listUsers.get(i).getUserId() %>&flag=1">点击跳转</a></td>			        
