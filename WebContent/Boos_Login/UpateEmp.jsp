...
<script src="/attendance/Boos_Login/js/jquery-1.11.1.min.js" type="text/javascript"></script>
...
<%
	...
	if(listUsers==null){
		...
		if(request.getParameter("userId")==null){
			...
			response.sendRedirect("/attendance/ListUserServlet?userId="+userId+"&flag=2");
			return;
...
$(function(){
	$(".confirmUser").click(function(){
		...
		var userClock = $("#userclock").val();
		var userPost2 = $("#userpost2").val();
		var userClock2 = $("#userclock2").val();
		var userId = $("#userId").val();
		$.ajax({
			url:"/attendance/UpdUserServlet",
			type:"get",
			data:{"userpost":userPost,
				"userclock":userClock,
				"userPost2":userPost2,
				"userId":userId,
				"userClock2":userClock2
			},
			success:function(data){
				alert("成功");
				location.href = "/attendance/Boos_Login/AllEmpQuery.jsp";
			  ...
			  <div class="bs-example1" data-example-id="contextual-table">
			    <table class="table table-hover">
			      ...
			      <tbody>
			        <tr>
			          <th>1</th>
			          <td><%=listUsers.get(0).getUserId()%><input type="hidden" value="<%=listUsers.get(0).getUserId()%>" id="userId"/></td>
			          <td><%=listUsers.get(0).getUserName() %></td>
			          <td><%=listUsers.get(0).getUserPost() %><input type="hidden" value="<%=listUsers.get(0).getUserPost() %>" id="userpost2"/><input type="text" class="inputabc" id="userpost"/></td>
			          <td><%=listUsers.get(0).getUserSal() %><input type="hidden" value="<%=listUsers.get(0).getUserSal() %>" id="userclock2"/><input type="text" class="inputabc" id="userclock" /></td>
			          <td><%=listUsers.get(0).getClockNum() %></td>
			          <td><%=listUsers.get(0).getEntry_time() %></td>
			          <td><a href="" class="confirmUser">更改信息</a></td>
			        </tr>
			      </tbody>
			    </table>
			   </div>
