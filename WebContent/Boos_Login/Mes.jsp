			    ...
			    <table class="table table-hover">
			      ...
			      List<Message> list = (List<Message>)session.getAttribute("list");
		      		if(list == null && flag != 1){
		      			...
		      			response.sendRedirect("/attendance/GetEmpMessageServlet?method=emp&userId="+userId+"&flag=3");
		      			return;
		      		}
			     ...
			     <%
			      ...
			      <c:forEach items="${sessionScope.list }" var="list" varStatus="vs">
				        <tr>
				          ...
				          <td>${list.kind }</td>
				          <td>${list.time }</td>
				          <td><a href="/attendance/SetYorNServlet?feedBack=true&id=${list.id }">同意</a><br />
				          	  <a href="/attendance/SetYorNServlet?feedBack=false&id=${list.id }">驳回</a>
				          </td>
				          <td><a href="/attendance/Boos_Login/BossMes.jsp?message=${list.userMessage }">详情</a></td>
				        </tr>
			        </c:forEach>
